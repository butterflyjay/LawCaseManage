...
class EntityManager {
        ...
        if (status === types_1.EEntityState.NEW) {
            ...
            if (idValue) { //存在主键
                ...
                if (en) { //如果该实体已存在，则执行update
                    sqlAndValue = translator_1.Translator.entityToUpdate(entity, ignoreUndefinedValue);
                }
                else { //实体不存在，则执行insert
                    sqlAndValue = translator_1.Translator.entityToInsert(entity);
                }
            }
            else { //无主键
                //根据策略生成主键
            ...
            //更新到数据库
            let sqlAndValue = translator_1.Translator.entityToUpdate(entity, ignoreUndefinedValue);
            let r = await sqlexecutor_1.SqlExecutor.exec(this, sqlAndValue[0], sqlAndValue[1]);
            if (r === null) {
                return null;
            }
    ...
    async delete(entity, className) {
        let sqlAndValue = translator_1.Translator.toDelete(entity, className);
        if (sqlAndValue) {
        ...
        if (orm && orm.id) {
            ...
            switch (orm.id.generator) {
                case 'table':
                    ...
                    let query = this.createNativeQuery("select id_value from " + orm.id.table + " where id_name='" + fn + "'");
                    ...
                    //主键值+1并写回数据库
                    await sqlexecutor_1.SqlExecutor.exec(this, "update " + orm.id.table + " set id_value=" + value + " where id_name='" + fn + "'");
                    break;
                case 'uuid':
